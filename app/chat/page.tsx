...
export default function WhatsAppWebClone() {
  ...
  const chats = useMemo(() => {
    ...
    return Array.from(chatMap.entries()).map(([phone, messages]) => {
      ...
      if (lastMsgText.startsWith("Agent (")) {
        const endIdx = lastMsgText.indexOf("): ");
        if (endIdx > -1) lastMsgText = lastMsgText.substring(endIdx + 3);
      } else if (lastMsgText.startsWith("Agent: ")) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!messageInput.trim() || !selectedChatPhone || !staffUser) return;
      ...
      >
        {!selectedChat ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center bg-[#F0F2F5] border-b-[6px] border-[#25D366]">
            ...
            <h1 className="text-[32px] font-light text-[#41525D] mb-4">Gustosa Web</h1>
            <p className="text-[#667781] text-sm max-w-md text-center">
              Send and receive messages seamlessly.
              <br />
              Logged in as <span className="font-semibold">{staffUser}</span>.
            </p>
          </div>
        ) : (
          /* Active Chat */
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-[#EFEAE2] flex flex-col gap-2 relative">
              ...
              {selectedChat.messages.map((msg) => {
                ...
                let assignedStaff = "Agent";
                if (text.startsWith("Agent (")) {
                  const endIdx = text.indexOf("): ");
                  if (endIdx > -1) {
                    assignedStaff = text.substring(7, endIdx);
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {isAgent && assignedStaff !== "Agent" && (
                         <div className="text-[10px] text-gray-500 font-medium mb-0.5">{assignedStaff}</div>
                      )}
